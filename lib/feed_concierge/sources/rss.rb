...
module FeedConcierge
  module Sources
    ...
    class Rss
      def initialize(name:, feeds:, summary_max_chars: 600, max_age_days: nil, categories: nil, link_regex: nil,
        ...
        @link_regex = link_regex && Regexp.new(link_regex)
      ...
      def fetch(url)
        body = Http.get(url, user_agent: @user_agent)
      ...
      # link_regex keeps the items of an aggregator that point at one site.
      def wanted?(item)
        return false if @link_regex && !link_of(item).match?(@link_regex)
